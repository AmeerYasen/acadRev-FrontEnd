...
/**
 * Get evidence files for a specific response
 * @param {string|number} responseId - Response ID
 * @returns {Promise<Array>} List of evidence files
 */
export const getEvidence = async (responseId) => {
  try {
    if (!responseId) {
      throw new Error('Response ID is required to get evidence');
    }
    
    console.log('getEvidence: Making API call for response:', responseId);
    const data = await apiFetch(QUALITATIVE_ENDPOINTS.GET_EVIDENCE(responseId));
    console.log('getEvidence: Received data:', data);
    
    if (!data || !Array.isArray(data)) {
      throw new Error('Invalid response format from evidence endpoint');
    }
    
    return data;
  } catch (error) {
    console.error('getEvidence: API call failed:', error);
...
/**
 * ========================================
 * QUALITATIVE SCORING ENDPOINTS
 * For Results Page Implementation
 * ========================================
 */
...
export const fetchDomainWeights = async () => {
  try {
    console.log('fetchDomainWeights: Making API call to', QUALITATIVE_ENDPOINTS.GET_DOMAIN_WEIGHTS);
    const data = await apiFetch(QUALITATIVE_ENDPOINTS.GET_DOMAIN_WEIGHTS);
    console.log('fetchDomainWeights: Received data:', data);
    ...
    console.error('fetchDomainWeights: API call failed:', error);
...
export const fetchDomainScores = async (programId) => {
  try {
    ...
    console.log('fetchDomainScores: Making API call for program:', programId);
    const data = await apiFetch(QUALITATIVE_ENDPOINTS.GET_DOMAIN_SCORES(programId));
    console.log('fetchDomainScores: Received data:', data);
    ...
    console.error('fetchDomainScores: API call failed:', error);
...
export const fetchWeightedResults = async (programId) => {
  try {
    ...
    console.log('fetchWeightedResults: Making API call for program:', programId);
    const data = await apiFetch(QUALITATIVE_ENDPOINTS.GET_WEIGHTED_RESULTS(programId));
    console.log('fetchWeightedResults: Received data:', data);
    ...
    console.error('fetchWeightedResults: API call failed:', error);
...
export const fetchCompleteQualitativeAnalysis = async (programId) => {
  try {
    ...
    console.log('fetchCompleteQualitativeAnalysis: Loading complete analysis for program:', programId);
    ...
    console.log('fetchCompleteQualitativeAnalysis: Complete analysis loaded:', analysis);
    return analysis;
  } catch (error) {
    console.error('fetchCompleteQualitativeAnalysis: Failed to load complete analysis:', error);
