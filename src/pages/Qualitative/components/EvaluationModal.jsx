...
const EvaluationModal = React.memo(
  ({
    ...
    const evaluationOptions = [
      {
        value: 2,
        label: translateQualitative("evaluationOptions.yes"),
        color: "bg-green-600 hover:bg-green-700",
      },
      {
        value: 1,
        label: translateQualitative("evaluationOptions.maybe"),
        color: "bg-yellow-600 hover:bg-yellow-700",
      },
      {
        value: 0,
        label: translateQualitative("evaluationOptions.no"),
        ...
        if (response && response.id) {
          for (const file of files) {
            try {
              ...
              showToast(
                `Evidence "${file.name}" uploaded successfully`,
                "success"
              );
            } catch (error) {
          ...
          showToast(
            "Please save your response first before uploading evidence",
            "warning"
          );
        }
      }
    ...
    const removeFile = (indicatorId, fileIndex) => {
      setSelectedFiles((prev) => ({
        ...prev,
        [indicatorId]:
          prev[indicatorId]?.filter((_, index) => index !== fileIndex) || [],
      }));
    };
    ...
    return (
      <div className="fixed inset-0 z-100 flex items-center justify-center">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900">
                  {translateQualitative("modal.title")}
                </h2>
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  ...
                  <p className="mt-4 text-gray-600">
                    {translateQualitative("loading.indicators")}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center h-full text-gray-500">
                <div className="text-center">
                  ...
                  <p className="text-lg font-medium">
                    {translateQualitative("modal.noDataAvailable")}
                  </p>
                  <p className="text-sm">
                    {translateQualitative("modal.contactAdmin")}
                  </p>
                </div>
              </div>
            ) : (
                ...
                <div className="mb-6">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-medium text-gray-700">
                      {translateQualitative("modal.progress")}
                    </span>
                    <span className="text-sm text-gray-600">
                      {progress[selectedDomain] || 0}%{" "}
                      {translateQualitative("complete")}
                    </span>
                  </div>
                  <Progress
                    value={progress[selectedDomain] || 0}
                    className="h-2"
                  />
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mb-4">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {evaluationOptions.map((option) => {
                              const isSelected =
                                response?.evaluation == option.value;
                              return (
                                <button
                                  key={option.value}
                                  onClick={() =>
                        ...
                        <div className="mb-4 space-y-3">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                              ...
                              >
                                ...
                                {showNotesFor[indicator.id]
                                  ? translateQualitative("hideNotes")
                                  : translateQualitative("showNotes")}
                              </Button>
                          ...
                          {selectedFiles[indicator.id] &&
                            selectedFiles[indicator.id].length > 0 && (
                              <div className="space-y-2">
                                <h4 className="text-sm font-medium text-gray-700">
                                  Selected Files (
                                  {selectedFiles[indicator.id].length})
                                </h4>
                                <div className="space-y-1 max-h-32 overflow-y-auto">
                                  {" "}
                                  {selectedFiles[indicator.id].map(
                                    (file, index) => {
                                      const uploadStatusKey = `${indicator.id}-${file.name}`;
                                      const fileUploadStatus =
                                        uploadStatus[uploadStatusKey];
          ...
          <div className="border-t border-gray-200 p-6">
            <div className="flex justify-between items-center">
              <div className="text-sm text-gray-600">
                {domainIndicators.length} indicators •{" "}
                {
                  Object.keys(responses).filter((key) =>
                    key.startsWith(`${selectedDomain}-`)
                  ).length
                }{" "}
                evaluated
                {Object.keys(unsavedChanges || {}).length > 0 && (
        ...
        <EvidencePopup
          ...
          responseId={
            currentIndicatorId
              ? responses[`${selectedDomain}-${currentIndicatorId}`]?.id
              : null
          }
        />
      </div>
