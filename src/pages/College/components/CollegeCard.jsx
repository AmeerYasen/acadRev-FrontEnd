    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow"> {/* Added flex flex-col flex-grow */}
        <div className="flex-grow"> {/* This div will take up available space */}
          ...
          <div className="text-sm text-text-muted mb-1">{translateCollege('card.established')}: {college.established || translateCollege('card.notAvailable')}</div>
          {college.head_name && (
            <div className="text-sm text-text-muted mb-2">{translateCollege('card.head')}: {college.head_name}</div>
          )}
        </div>
        <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-100">
          ...
          <span className="text-xs px-2 py-1 bg-gray-100 rounded-full text-text-muted">
            {college.departments?.length || 0} {translateCollege('card.departments')}
          </span>
        </div>
      </div>
    </div>
