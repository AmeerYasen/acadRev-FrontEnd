...
const ProgramEditModal = ({ program, isOpen, onClose, onUpdate, userRole }) => {
  ...
  const renderField = (label, value, name, type = "text", multiline = false) => {
    const getPlaceholder = () => {
      switch(name) {
        case 'name': return translatePrograms('modal.placeholders.enterProgramName');
        case 'language': return translatePrograms('modal.placeholders.selectLanguage');
        case 'website': return translatePrograms('modal.placeholders.enterWebsite');
        case 'head_name': return translatePrograms('modal.placeholders.enterCoordinator');
        case 'established': return translatePrograms('modal.placeholders.enterDuration');
    ...
    if (name === "name" && !isEditing) {
      return (
        <div className="mb-6">
          ...
          <p className="text-gray-900 font-medium">{value || translatePrograms('modal.fields.notSpecified')}</p>
        </div>
      );
    }
        ...
        return (
          <div className="mb-6">
            ...
            <select
              ...
              className="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 text-base transition-all duration-200"
            >
              <option value="">{translatePrograms('modal.placeholders.selectLanguage')}</option>
              <option value="English">{translatePrograms('modal.fields.languages.english')}</option>
              <option value="Arabic">{translatePrograms('modal.fields.languages.arabic')}</option>
              <option value="French">{translatePrograms('modal.fields.languages.french')}</option>
              <option value="German">{translatePrograms('modal.fields.languages.german')}</option>
            </select>
          </div>
        );
      }
    ...
    return (
      <div className="mb-6">
        <h3 className="text-sm font-medium text-gray-500 mb-2">{label}</h3>
        <p className="text-gray-900">{value || translatePrograms('modal.fields.notSpecified')}</p>
      </div>
    );
  };
        ...
        <div className="relative mb-8 pt-6">
          <div className="flex flex-col items-center text-center">
            ...
            <h2 className="text-white text-2xl font-bold mb-2">
              {isEditing ? translatePrograms('modal.editTitle') : (editedProgram.name || translatePrograms('modal.title'))}
            </h2>
          ...
          <form onSubmit={handleUpdateSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              <div className="space-y-6">
                {renderField(translatePrograms('modal.fields.programName'), editedProgram.name, "name")}
                {renderField(translatePrograms('modal.fields.language'), editedProgram.language, "language")}
                {renderField(translatePrograms('modal.fields.website'), editedProgram.website, "website", "url")}
              </div>
              <div className="space-y-6">
                {renderField(translatePrograms('modal.fields.coordinator'), editedProgram.head_name, "head_name")}
                {renderField(translatePrograms('modal.fields.established'), editedProgram.established, "established", "number")}
              </div>
            </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              <div className="space-y-6">
                {renderField(translatePrograms('modal.fields.programName'), editedProgram.name, "name")}
                {renderField(translatePrograms('modal.fields.language'), editedProgram.language, "language")}
                {renderField(translatePrograms('modal.fields.website'), editedProgram.website, "website")}
                {renderField(translatePrograms('modal.fields.coordinator'), editedProgram.head_name, "head_name")}
                {renderField(translatePrograms('modal.fields.established'), formattedDate, "created_at")}
              ...
              <div className="space-y-6">
                <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                  <h3 className="text-sm font-medium text-gray-700 mb-4 flex items-center">
                    ...
                    {translatePrograms('modal.fields.department')} {translatePrograms('modal.fields.information')}
                  </h3>
                  <div className="space-y-3">
                    {program.department_name && (
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-600">{translatePrograms('modal.fields.department')}:</span>
                        <span className="text-sm text-gray-800">{program.department_name}</span>
                      </div>
                    )}
                    {program.college_name && (
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-600">{translatePrograms('modal.fields.college')}:</span>
                        <span className="text-sm text-gray-800">{program.college_name}</span>
                      </div>
                    )}
                    {program.university_name && (
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-600">{translatePrograms('modal.fields.university')}:</span>
                        <span className="text-sm text-gray-800">{program.university_name}</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            {!isEditing && (
              <div className="pt-6 mt-6 border-t border-gray-200">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">{translatePrograms('modal.assessmentTools.title')}</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  >
                    ...
                    <span className="font-medium">{translatePrograms('modal.assessmentTools.quantitativeIndicators')}</span>
                  </button>
                  ...
                  >
                    <Pencil className="w-4 h-4 group-hover:scale-110 transition-transform" />
                    <span className="font-medium">{translatePrograms('modal.assessmentTools.qualitativeIndicators')}</span>
                  </button>
                  ...
                  >
                    <Pencil className="w-4 h-4 group-hover:scale-110 transition-transform" />
                    <span className="font-medium">{translatePrograms('modal.assessmentTools.selfAssessmentReport')}</span>
                  </button>
                </div>
              </div>
            )}
