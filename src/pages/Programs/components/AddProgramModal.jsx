...
const AddProgramModal = ({ isOpen, onClose, onAddProgram, userRole, departmentId, collegeId, universityId }) => {  
  const { translatePrograms } = useNamespacedTranslation();  const [newProgram, setNewProgram] = useState({
    name: '',
    language: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!newProgram.name || !newProgram.language) {
      setError(translatePrograms('errors.createError'));
      return;
    }
    ...
    try {
      ...
      setError(err.message || translatePrograms('errors.createError'));
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-100 p-4 transition-opacity duration-300 ease-in-out">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 sm:p-8 transform transition-all duration-300 ease-in-out scale-100">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-gray-800">{translatePrograms('modal.addTitle')}</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="programName" className="block text-sm font-medium text-gray-700 mb-1">
              {translatePrograms('modal.fields.programName')} <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              name="name"
              ...
              placeholder={translatePrograms('modal.placeholders.enterProgramName')}
            />
          </div>          <div>
            <label htmlFor="programLanguage" className="block text-sm font-medium text-gray-700 mb-1">
              {translatePrograms('modal.fields.language')} <span className="text-red-500">*</span>
            </label>
            <select
              name="language"
              id="programLanguage"
          ...
          <div className="flex flex-col sm:flex-row-reverse gap-3 pt-6 border-t mt-6">
            ...
            >
              {translatePrograms('modal.actions.addProgram')}
            </button>
            <button
              type="button"
              onClick={onClose}
            ...
            >
              {translatePrograms('actions.cancel')}
            </button>
          </div>
        </form>
      </div>
