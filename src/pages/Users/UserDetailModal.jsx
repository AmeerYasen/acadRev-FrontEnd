...
function UserDetailModal({
  ...
  const canDelete =
    loggedInUser?.role === ROLES.ADMIN && user.role === ROLES.AUTHORITY;
        ...
        <p className="text-sm text-gray-800 font-medium">
          {value === null || value === undefined || value === "" ? (
            <span className="italic text-gray-400">
              {translateUsers("fields.notAvailable")}
            </span>
          ) : (
            value
          )}
    ...
    <div className="fixed inset-0 bg-gray-900/75 flex justify-center items-center z-100 p-4 transition-opacity duration-300">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg relative animate-fade-in-up">
        <button
          ...
          aria-label={translateUsers("modal.closeModal")}
        >
          <X size={24} />
        </button>
        ...
        <div className="p-5 max-h-[60vh] overflow-y-auto">
          ...
          {detailItem(
            RoleIcon,
            translateUsers("fields.role"),
            translateUsers(`roles.${user.role}`)
          )}
          {detailItem(
            Briefcase,
            translateUsers("fields.authorityName"),
            user.authority_name
          )}
          {detailItem(
            Key,
          ...
          {detailItem(
            University,
            translateUsers("fields.universityId"),
            user.university_id
          )}
          {detailItem(
            School,
            translateUsers("fields.collegeId"),
            user.college_id
          )}
          {detailItem(
            Building,
            translateUsers("fields.departmentId"),
            user.department_id
          )}
          {detailItem(
            CheckSquare,
            translateUsers("fields.active"),
            user.is_active
              ? translateUsers("messages.yes")
              : translateUsers("messages.no")
          )}
          {detailItem(
            UsersIcon,
            translateUsers("fields.createdAt"),
            new Date(user.created_at).toLocaleString()
          )}
          {detailItem(
            Image,
            translateUsers("fields.imageUrl"),
            <div className="flex items-center space-x-3">
              <span className="text-sm text-gray-600">
                {user.image_url
                  ? translateUsers("messages.customImage")
                  : translateUsers("messages.placeholderImage")}
              </span>
            </div>
          )}
        </div>
        <div className="flex justify-between items-center p-4 bg-gray-50 border-t border-gray-200 rounded-b-lg">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 bg-white hover:bg-gray-100 border border-gray-300 text-gray-700 text-sm font-medium rounded-md shadow-sm"
          >
