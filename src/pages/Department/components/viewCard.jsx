...
const DepartmentDisplayCard = ({ 
  ...
  const departmentName = item.name || 'Unnamed Department';
  
  let establishedDisplay = 'N/A';
  if (item.created_at) {
    try {
  ...
  const programCount = item.program_count || 0;
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow"> {/* Use flex-grow to make content area fill height */}
        <div className="flex-grow space-y-2"> {/* Added space-y for consistent spacing */}
          ...
          {departmentHead && (
            <div className="text-sm text-gray-600">
              <span className="font-medium text-gray-500">Chair:</span> {departmentHead}
            </div>
          )}
        ...
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-100"> {/* Added more top margin and padding */}
          <div className="text-xs text-primary-dark font-medium">DEPARTMENT</div> {/* Static type display */}
