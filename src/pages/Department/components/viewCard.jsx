...
const DepartmentDisplayCard = ({ 
  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow"> {/* Use flex-grow to make content area fill height */}
        <div className="flex-grow space-y-2"> {/* Added space-y for consistent spacing */}
          <div className="flex justify-between items-start mb-3"> {/* Increased bottom margin */}
          ...
          {departmentHead && (
            <div className="text-sm text-gray-600">
              <span className="font-medium text-gray-500">{translateDepartment('viewCard.chair')}:</span> {departmentHead}
            </div>
          )}
          ...
          {item.college_name && (
             <div className="text-sm text-gray-600">
               <span className="font-medium text-gray-500">{translateDepartment('viewCard.college')}:</span> {item.college_name}
             </div>
          )}
           {item.university_name && (
             <div className="text-sm text-gray-600">
               <span className="font-medium text-gray-500">{translateDepartment('viewCard.university')}:</span> {item.university_name}
             </div>
          )}
        </div>
        
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-100"> {/* Added more top margin and padding */}
          <div className="text-xs text-primary-dark font-medium">{translateDepartment('viewCard.type')}</div> {/* Static type display */}
          <span className="text-xs px-3 py-1 bg-primary/10 text-primary rounded-full font-medium"> {/* Enhanced styling for count */}
            {translateDepartment('viewCard.programCount', { count: programCount })}
          </span>
        </div>
      </div>
    </div>
